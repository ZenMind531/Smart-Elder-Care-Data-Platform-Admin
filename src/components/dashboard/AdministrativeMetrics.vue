  ...
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 md:gap-6">
    ...
    >
      <div class="flex items-start justify-between gap-4">
        ...
        >
          <component :is="metric.icon" class="size-6" :class="toneClassMap[metric.tone].icon" />
        </div>
        <span
      ...
      <div class="mt-5">
        ...
        <div class="mt-2 flex items-end gap-1">
          ...
          <span class="mb-1 text-sm font-medium text-gray-500 dark:text-gray-400">{{ metric.unit }}</span>
        </div>
        <p class="mt-2 text-theme-xs text-gray-500 dark:text-gray-400 text-pretty">{{ metric.detail }}</p>
      </div>
    </article>
  </div>
</template>
