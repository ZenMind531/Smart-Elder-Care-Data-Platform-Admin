...
| careContent | String | 是 | 护理内容 |
| careTime | DateTime | 是 | 护理时间，格式 yyyy-MM-dd HH:mm:ss |
| remark | String | 否 | 备注 |
...
| serviceType | String | 否 | health_check/home_care/rehabilitation/consultation/other |
...
| appointmentTime | DateTime | 是 | 预约时间，格式 yyyy-MM-dd HH:mm:ss |
...
返回字段包含：`id`、`elderlyId`、`elderlyName`、`serviceType`、`serviceDate`、`serviceTime`、`remark`、`status`、`createTime`。
